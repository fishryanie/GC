...
export const PriceProfileCreateForm = forwardRef<PriceProfileCreateFormHandle, PriceProfileCreateFormProps>(function PriceProfileCreateForm(
  ...
  const watchedPrices = Form.useWatch('prices', form) as Record<string, number | null> | undefined;

  const hasValidPrice = useMemo(() => products.some(product => Number(watchedPrices?.[product.id] ?? 0) > 0), [products, watchedPrices]);
  const canSubmit = products.length > 0 && hasValidPrice && watchedName.trim().length > 0;
  const availableProfileTypes = useMemo(() => (canManageCost ? PRICE_PROFILE_TYPES : (['SALE'] as PriceProfileType[])), [canManageCost]);
  ...
  function handleFinish(values: PriceProfileCreateFormValues) {
    ...
    Modal.confirm({
      ...
      cancelButtonProps: {
        className: '!border-border !bg-background-secondary !text-foreground hover:!border-primary-500/40',
      },
      onOk: async () => {
  ...
  const handleGenerateTestData = useCallback(() => {
    ...
    const currentType = canManageCost ? (form.getFieldValue('type') as PriceProfileType | undefined) || 'SALE' : 'SALE';
    const [minPrice, maxPrice] = currentType === 'COST' ? [80000, 190000] : [100000, 280000];

    const generatedPrices = Object.fromEntries(products.map(product => [product.id, roundToThousand(faker.number.int({ min: minPrice, max: maxPrice }))]));
  ...
  return (
    <Form
      ...
      <div className='grid grid-cols-2 gap-2.5 max-md:grid-cols-1'>
        {canManageCost ? (
          <Form.Item name='type' label={t('type')} rules={[{ required: true }]} className='mb-0'>
            <Select
      ...
      {!canManageCost ? (
        <div className='rounded-lg border border-border bg-background-tertiary px-3 py-2 text-xs text-foreground-secondary'>{t('saleOnlyHint')}</div>
      ) : null}
      ...
      <section className='flex min-h-0 flex-1 flex-col gap-2'>
        <div className='flex items-center justify-between gap-3'>
          <span className='text-[13px] font-semibold text-foreground-secondary'>{t('priceByProduct')}</span>
          <span className='text-[13px] text-foreground-muted'>{t('productsCount', { count: products.length })}</span>
        </div>

        <div className='min-h-0 flex-1 overflow-y-auto rounded-lg border border-border bg-background-tertiary p-2.5'>
          {products.length === 0 ? (
            <p className='m-0 text-[13px] text-foreground-muted'>{t('emptyProducts')}</p>
          ) : (
              ...
              {products.map(product => (
                <div
                  ...
                  <Space.Compact style={{ width: '100%' }}>
                    <Form.Item name={['prices', product.id]} style={{ marginBottom: 0, width: '100%' }}>
                      <InputNumber
                        min={0}
                        step={1000}
                        precision={0}
                    ...
                    <Input readOnly value={`${tCommon('vnd')}/kg`} style={{ width: 200, textAlign: 'center' }} />
                  </Space.Compact>
                </div>
              ))}
            </div>

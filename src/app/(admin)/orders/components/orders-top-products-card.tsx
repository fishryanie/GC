...
type OrdersTopProductsSort = 'REVENUE' | 'ORDERS' | 'WEIGHT';

type OrdersTopProductsCardProps = {
...
function getSortMetricValue(item: SellerDetailsTopProduct, sortBy: OrdersTopProductsSort) {
  if (sortBy === 'ORDERS') {
...
export function OrdersTopProductsCard({ products, canViewCost, className }: OrdersTopProductsCardProps) {
  const t = useTranslations('ordersPage');
  const locale = useLocale();
  const [isExpanded, setIsExpanded] = useState(false);
  const [sortBy, setSortBy] = useState<OrdersTopProductsSort>('REVENUE');
  ...
  return (
    <article className={`flex h-full min-h-0 min-w-0 flex-col overflow-hidden rounded-xl border border-border bg-background-secondary p-4${resolveClassName(className)}`}>
      <h3 className='m-0 text-sm font-semibold text-foreground'>{t('charts.topProductsPanelTitle')}</h3>
      <p className='m-0 mt-1 text-xs text-foreground-secondary'>{t('charts.topProductsPanelHint')}</p>
      ...
      {!sortedProducts.length ? (
        ...
        <>
          <div className='mt-2 flex items-center gap-1.5 overflow-x-auto pb-1'>
            <span className='shrink-0 text-[11px] text-foreground-muted'>{t('charts.sortByLabel')}</span>
            {([
              ['REVENUE', t('charts.sortRevenue')],
              ['ORDERS', t('charts.sortOrders')],
              ['WEIGHT', t('charts.sortWeight')],
          ...
          {sortedProducts.length > DEFAULT_VISIBLE_COUNT ? (
            <div className='mt-3 border-t border-border pt-3'>
              <button
                ...
                {isExpanded ? t('charts.showTopProducts', { count: DEFAULT_VISIBLE_COUNT }) : t('charts.showAllProducts', { count: sortedProducts.length })}
              </button>
            </div>
          ) : null}
        </>

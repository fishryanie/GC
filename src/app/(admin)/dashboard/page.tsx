...
export default async function DashboardPage({
  ...
  const [tDashboard, tStatuses] = await Promise.all([getTranslations('dashboard'), getTranslations('statuses')]);
  const session = await requireAuthSession();
  const canViewCost = session.seller.role === 'ADMIN';

  const [stats, recentOrders] = await Promise.all([getDashboardStats(), listOrders({ limit: 6 })]);
    ...
    <Space orientation='vertical' size={16} style={{ width: '100%' }}>
      <Card className='rounded-xl border border-border bg-background-secondary'>
        <p className='m-0 inline-flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.12em] text-foreground-muted'>
          <span className='inline-flex h-6 w-6 items-center justify-center rounded-full bg-background-tertiary text-[13px]' aria-hidden>
            📊
          </span>
          Dashboard
        </p>
        <h2 className='mb-1 mt-2 text-3xl font-bold leading-tight text-foreground'>{tDashboard('title')}</h2>
        <p className='m-0 text-foreground-secondary'>{tDashboard('subtitle')}</p>
      </Card>
      ...
      <Row gutter={[16, 16]}>
        <Col xs={24} md={12} xl={6}>
          <Card className='border border-border bg-background-secondary'>
            ...
            {canViewCost ? <p className='m-0 text-xs text-foreground-muted'>{tDashboard('profit', { amount: formatCurrency(stats.totalProfit) })}</p> : null}
          </Card>
        </Col>
        <Col xs={24} md={12} xl={6}>
          <Card className='border border-border bg-background-secondary'>
            <Statistic title={tDashboard('activeOrders')} value={stats.deliveringOrders} />
            <p className='m-0 text-xs text-foreground-muted'>{tDashboard('totalOrders', { count: stats.totalOrders })}</p>
          </Card>
        </Col>
        <Col xs={24} md={12} xl={6}>
          <Card className='border border-border bg-background-secondary'>
            <Statistic title={tDashboard('activeProducts')} value={stats.activeProducts} />
            <p className='m-0 text-xs text-foreground-muted'>{tDashboard('totalProducts', { count: stats.totalProducts })}</p>
          </Card>
        </Col>
        <Col xs={24} md={12} xl={6}>
          <Card className='border border-border bg-background-secondary'>
            <Statistic title={tDashboard('ordersNotCollected')} value={stats.uncollectedOrders} />
            <p className='m-0 text-xs text-foreground-muted'>{tDashboard('supplierUnpaid', { count: stats.unpaidOrders })}</p>
          </Card>
        </Col>
      </Row>
          ...
          <Card
            ...
            <div className='overflow-x-auto'>
              <table className='w-full min-w-[860px] border-collapse'>
                <thead>
                  <tr>
                    <th className='border-b border-border px-1.5 py-2 text-left'>{tDashboard('table.orderCode')}</th>
                    <th className='border-b border-border px-1.5 py-2 text-left'>{tDashboard('table.buyer')}</th>
                    <th className='border-b border-border px-1.5 py-2 text-left'>{tDashboard('table.deliveryDate')}</th>
                    <th className='border-b border-border px-1.5 py-2 text-left'>{tDashboard('table.weight')}</th>
                    <th className='border-b border-border px-1.5 py-2 text-left'>{tDashboard('table.bill')}</th>
                    {canViewCost ? <th className='border-b border-border px-1.5 py-2 text-left'>{tDashboard('table.profit')}</th> : null}
                    <th className='border-b border-border px-1.5 py-2 text-left'>{tDashboard('table.status')}</th>
                  </tr>
                </thead>
                <tbody>
                  {recentOrders.map(order => (
                    <tr key={order.id}>
                      ...
                      {canViewCost ? <td className='border-b border-border px-1.5 py-2 text-green-500'>{formatCurrency(order.totalProfitAmount)}</td> : null}
          ...
          <Space orientation='vertical' size={16} style={{ width: '100%' }}>
            ...
            {canViewCost ? (
              <Card className='border border-border bg-background-secondary' title={tDashboard('seedData')}>
                <p className='text-foreground-secondary'>{tDashboard('seedDescription')}</p>

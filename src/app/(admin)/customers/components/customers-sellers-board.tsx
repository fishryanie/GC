    ...
    <>
        ...
        <Link
          ...
          className={`inline-flex items-center gap-2 rounded-lg px-4 py-2 text-sm font-medium transition-colors ${
            activeTab === "customers"
              ...
              : "!bg-[#171717] !text-[#a3a3a3] hover:!bg-[#262626] hover:!text-[#fafafa]"
          }`}
        >
      ...
      {activeTab === "customers" ? (
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex min-w-0 gap-3">
                      ...
                      <div className="min-w-0">
                        ...
                        {customer.notes ? (
                          <p className="mt-2 text-sm text-foreground-secondary">{customer.notes}</p>
                        ) : null}
                      </div>
                    </div>
        ...
        <section className="space-y-4">
          <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
            <StatCard
              label={t("sellers.stats.total")}
              value={sellersData.totalCount}
              icon={<Users className="h-4 w-4 text-emerald-300" />}
              tone="emerald"
            />
            <StatCard
              label={t("sellers.stats.enabled")}
              value={sellersData.enabledCount}
              icon={<CheckCircle2 className="h-4 w-4 text-sky-300" />}
              tone="sky"
            />
            <StatCard
              label={t("sellers.stats.disabled")}
              value={sellersData.disabledCount}
              icon={<Lock className="h-4 w-4 text-amber-300" />}
              tone="amber"
            />
            <StatCard
              label={t("sellers.stats.admins")}
              value={sellersData.adminCount}
            ...
            <section className="rounded-xl border border-border bg-background-secondary p-4">
              ...
              <div className="grid gap-2 lg:grid-cols-2">
                {sellersData.topSellers30d.map((item) => (
                  <div
                    key={item.sellerId}
                    className="flex items-center justify-between rounded-lg border border-border bg-background-tertiary px-3 py-2"
                  >
                      ...
                      <div className="flex min-w-0 gap-3">
                        ...
                        <div className="min-w-0">
                          ...
                          <p className="mt-1 text-xs text-foreground-muted">
                            {seller.lastLoginAt
                              ? t("sellers.lastLogin", { value: formatDateTime(seller.lastLoginAt) })
                              : t("sellers.noLogin")}
                          </p>
                        </div>
                      </div>
      ...
      {customerModalMode ? (
        ...
        >
          <form
            key={`${customerModalMode}-${editingCustomer?.id ?? "new"}`}
            action={upsertCustomerAction}
            className="space-y-3"
          >
            {customerModalMode === "edit" && editingCustomer ? (
            ...
            <ModalActions
              ...
              submitLabel={
                customerModalMode === "edit" ? t("customers.modal.submitEdit") : t("customers.modal.submitCreate")
              }
              onCancel={() => {
                setCustomerModalMode(null);
                setEditingCustomer(null);
      ...
      {sellerModalMode && isAdmin ? (
        ...
        >
          <form
            key={`${sellerModalMode}-${editingSeller?.id ?? "new"}`}
            action={upsertSellerAction}
            className="space-y-3"
          >
            {sellerModalMode === "edit" && editingSeller ? (
              <input type="hidden" name="id" value={editingSeller.id} />
            ) : null}
            ...
            <Field label={t("sellers.modal.role")}>
              <select
                name="role"
                defaultValue={editingSeller?.role ?? "SELLER"}
                className="focus-ring h-10 w-full rounded-lg border border-border bg-background-tertiary px-3 text-sm text-foreground"
              >
            ...
            <ModalActions
              cancelLabel={t("modal.cancel")}
              submitLabel={
                sellerModalMode === "edit" ? t("sellers.modal.submitEdit") : t("sellers.modal.submitCreate")
              }
              onCancel={() => {
                setSellerModalMode(null);
                setEditingSeller(null);
      ...
      {resetTarget && isAdmin ? (
        <ModalShell
          title={t("sellers.resetModal.title", { name: resetTarget.name })}
        ...
        >
          <form action={resetSellerPasswordAction} className="space-y-3">
            ...
            <ModalActions
              cancelLabel={t("modal.cancel")}
              submitLabel={t("sellers.resetModal.submit")}
